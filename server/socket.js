...
const { doUploadImage } = require("./uploadImg");
const { doLogin, register } = require("./account");
...
function init(io) {
    io.on("connection", function(socket) {
        ...
        socket.on("upload", async ({ data, name, id }, cb) => {
        ...
        $on(
            ...
            async function({ id, uid }) {
                ...
                const item = todo.getTodo(id);
                const message = `【${topUserMap.getName(
                    socket.context.uid
                )}】把任务：【${item.title}】移交给了你。
详情参见 ${getViewUrl(id)}`;
...
详情参见 ${getViewUrl(id)}`;
        ...
        function checkAuthority(item, uid) {
            if (
                uid != item.owner &&
                uid != item.requestor &&
                !(item.partners && item.partners[uid])
            ) {
                return false;
            }
            return true;
        ...
        $on(
            ...
            async function({ id, uid }) {
                const item = todo.getTodo(id);
                if (!checkAuthority(item, socket.context.uid)) {
                    throw "no authority";
                }
                if (!item.partners) {
                    item.partners = {};
...
详情参见 ${getViewUrl(id)}`;
        ...
        $on(
            ...
            async function(id) {
                const item = todo.getTodo(id);
                item.status = "done";
                const message = `【${item.title}】任务被【${topUserMap.getName(
                    socket.context.uid
                )}】完成了哦～  
详情参见 ${getViewUrl(id)}`;
        ...
        $on(
            ...
            async function(item) {
                ...
                await todo.edit(id, item);
                let message = `【${$requestor.name}】 给你分配了任务【${
                    item.title
                }】
详情参见 ${getViewUrl(id)}`;
                ...
                await new Promise(resolve => {
                    const dir = `./public/files/${topUserMap.getName(
                        socket.context.uid
                    )}`;
