<template>
    <div class="todo-editor">
        ...
        <div class="input-block">
            ...
            <button v-if="mode==='Create'" class="btn btn-primary" style="margin-top:10px;" @click="assignToYourself">{{$t('Assign to yourself')}}</button>
        </div>
        <div class="input-block">
            <label>
...
export default {
    ...
    methods: {
        assignToYourself() {
            this.owner = store.state.user.uid;
        },
        backup() {
        ...
        set(info) {
            ...
            this.commonTags = JSON.parse(
                JSON.stringify(store.state.commonTags)
            );
            ...
            if (info.owner) {
                this.targetOwner = this.userList.find(
                    user => user.value == info.owner
                );
            }
